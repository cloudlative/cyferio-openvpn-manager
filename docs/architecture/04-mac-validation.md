...
2. If the user has zero MACs registered: accepts (`no_mac_policy`) unless `mac_required=true`, in which case it rejects instead (`no_mac_registered`). Default `mac_required=false` — MAC binding is opt-in per user, not a blanket requirement; setting it `true` makes registration itself mandatory for every user. Deliberately a separate config key from `mac_enforcement_mode` below, not a third value of it — that setting only ever governed an *already-registered* user's fallback behavior, and folding the zero-MACs case into "strict" would have silently changed what an existing deployment's `strict` setting does on upgrade, with no config change of the operator's own.
