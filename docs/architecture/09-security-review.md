...
| PKI protection | CA private key compromise = full trust compromise | CA key never leaves `/etc/cyferio/pki` (`0700`); no plaintext CA key in backups beyond the same-permission tarball; [`SECURITY.md`](../../SECURITY.md) documents recommended offline-CA posture for high-security deployments as a documented option, not forced default (keeps the default install simple per spec's ease-of-use goals) |
